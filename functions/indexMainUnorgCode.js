...
/**
 * Download attachment from QuickBooks and save to Cloud Storage + Firestore
 * Prevents duplicate downloads by checking if already exists
 */
async function downloadAndSaveQBOAttachment({
    tenantId,
    entityName,
    ...
    if (existingRef) {
        ...
        // If already has storage info and not marked as failed, skip download
        if (existingData?.storagePath && existingData?.downloadUrl && !existingData?.downloadFailed) {
            ...
            // Just update with latest QBO metadata
            await createAttachableRecordFromQBO({
                tenantId,
                entityName,
                entityDocId,
                entityQbId,
                qboAttachable,
                storageInfo: null, // Don't update storage info
    ...
    try {
        ...
        // Create/update Firestore record even if download fails (for tracking)
        await createAttachableRecordFromQBO({
            tenantId,
            entityName,
            entityDocId,
    ...
    try {
        ...
        // 3. Create/update Firestore record
        await createAttachableRecordFromQBO({
            tenantId,
            entityName,
            entityDocId,
...
async function createAttachableRecordFromQBO({
    ...
    // CRITICAL: Check if already exists by QBO ID
    const existingRef = await findDocRefByQbId(tenantId, "attachables", qbId);
    ...
    if (existingRef) {
        ...
        const updateData = cleanUndefined({
            ...
            // Update metadata only if storage info is provided
            ...(storageInfo ? {
                storagePath: storageInfo.storagePath,
                downloadUrl: storageInfo.downloadUrl,
